...
(function ($) {
	...
	var xsl = {
		...
		'stock' : '<' + '?xml version="1.0" encoding="utf-8" ?>' + 
			...
			'<xsl:template match="/">' + 
			'	<ul>' + 
			'	<xsl:for-each select="//element[not(@parent_id) or @parent_id=0 or not(@parent_id = //element/@id)]">' + /* the last `or` may be removed */
			'		<xsl:call-template name="nodes">' + 
			'			<xsl:with-param name="node" select="." />' + 
			'			<xsl:with-param name="is_last" select="number(position() = last())" />' + 
			'		</xsl:call-template>' + 
			...
			'	<xsl:param name="node" />' + 
			'	<xsl:param name="is_last" />' + 
			'	<xsl:variable name="children" select="count(//element[@parent_id=$node/attribute::id]) &gt; 0" />' + 
			'	<li>' + 
			'	<xsl:attribute name="class">' + 
			'		<xsl:if test="$is_last = true()">jstree-last </xsl:if>' + 
			'		<xsl:choose>' + 
